...
st.title("Genetic Algorithm for TSP with Custom Cities")

# Get cities and coordinates from the user
num_cities = st.number_input("Enter number of cities:", min_value=2, max_value=20, value=10, step=1)
city_data = {
    "City": [],
    "x": [],
    "y": []
...
for i in range(num_cities):
    ...
    y_coord = st.number_input(f"City {i + 1} y-coordinate:", value=float(i * 2 + 1))

    city_data["City"].append(city_name)
    city_data["x"].append(x_coord)
    city_data["y"].append(y_coord)

# Create a DataFrame and city coordinates dictionary
city_df = pd.DataFrame(city_data)
city_coords = dict(zip(city_df["City"], zip(city_df["x"], city_df["y"])))
...
# Genetic Algorithm Functions (kept the same as in your original code)
...
def mutation(offspring):
    index_1, index_2 = np.random.randint(0, len(cities_names), size=2)
...
def run_ga(cities_names, n_population, n_generations, crossover_per, mutation_per):
    ...
    for generation in range(n_generations):
        ...
        population = sorted(offspring_list + parents_list, key=total_dist_individual)[:n_population]
    return population

# Run GA and display best route
best_mixed_offspring = run_ga(cities_names, n_population, n_generations, crossover_per, mutation_per)
total_dist_all_individuals = [total_dist_individual(indiv) for indiv in best_mixed_offspring]
