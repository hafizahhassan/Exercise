...
def get_city_input():
    ...
    for i in range(10):
        cities_names = st.text_input(f"Enter name for city {i + 1}:", f"City_{i + 1}")
        cities_names.append(cities_names)

        x = st.number_input(f"Enter x-coordinate for {cities_names}:", min_value=1, max_value=10)
        y = st.number_input(f"Enter y-coordinate for {cities_names}:", min_value=1, max_value=10)
        
        city_coords[cities_names] = (x, y)
    
    return cities_names, city_coords
